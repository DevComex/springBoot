...
public class XWBLotteryService {
	private static final Logger logger = GYYXLoggerFactory
			.getLogger(XWBLotteryService.class);
	...
	public ResultBean<UserLotteryBean> lotteryByDataBase(int actionCode,
		...
		if (resultBean.getIsSuccess()
			...
			if (resultBean.getData().getIsReal().equals("card")) {
				RechargeBll rechargeBll = new RechargeBll();
			...
			userInfoParam.setAccount(userInfo.getAccount());
			userInfoParam.setLoginIP(userInfo.getIp());
			PictureUrlBean pictureUrlBean = pictureUrlBLL.getPictureUrlByPrizeCode(resultBean.getData().getPrizeCode());
			cn.gyyx.action.beans.novicecard.ResultBean<String> result = xWBService.sendRewardForLotteryAndExchange(serverCode, server, userInfoParam, resultBean.getData().getPrizeChinese()+"#"+pictureUrlBean.getPrizeYard(),pictureUrlBean.getMailContent());
			if(!result.getMessage().equals("领取成功！")){
