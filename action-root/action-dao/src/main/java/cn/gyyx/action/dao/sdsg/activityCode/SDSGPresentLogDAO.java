...
public class SDSGPresentLogDAO implements ISDSGPresentLogMapper {
	private static final Logger LOG = GYYXLoggerFactory.getLogger(SDSGPresentLogDAO.class);
	...
	public int presentCountDay(int actioinCode, String account, String endTime, String startTime) {
		...
		int result = -1;
		try {
			ISDSGPresentLogMapper iPresentLogMapper = session.getMapper(ISDSGPresentLogMapper.class);
	...
	public void updateGameActivityCode(int gameId, String activityCode,String serverID,String accountName,Date date,Integer userId) {
		...
		try {
			ISDSGPresentLogMapper iPresentLogMapper = session.getMapper(ISDSGPresentLogMapper.class);
			iPresentLogMapper.updateGameActivityCode(gameId, activityCode,serverID,accountName,date,userId);
		} catch (Exception e) {
	...
	public void updateGameActivityCodeStatus(String cardCode) {
		...
		try {
			ISDSGPresentLogMapper iPresentLogMapper = session.getMapper(ISDSGPresentLogMapper.class);
			iPresentLogMapper.updateGameActivityCodeStatus(cardCode);
		} catch (Exception e) {
			LOG.error("更新game_activity_code表失败！！", e);
		} finally {
			session.commit();
	...
	public int isRightActiveCode(String activeCode) {
		try (SqlSession session = getSession()) {
			ISDSGPresentLogMapper iPresentLogMapper = session.getMapper(ISDSGPresentLogMapper.class);
			return  iPresentLogMapper.isRightActiveCode(activeCode);
		}
