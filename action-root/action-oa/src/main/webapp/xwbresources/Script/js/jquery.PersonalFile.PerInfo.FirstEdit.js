...
$(function () {
    ...
    $(".js_savePerInfo").click(function () {
        if ($("#EditPerInfo").valid()) {
            $.ajax({
                url: 'http://oa.gyyx.cn/StaffEntryApplication/EditOfficeInfo',
                type: "post",
                //async: true,
                data: {
                    ...
                    OAPwd: $("input[name=OfficePassword]").val(),
                    ConfrimPassword: $("input[name=OfficePasswords]").val()
                },
                dataType: "json",
                success: function (d) {
                    alert(d.Message);
                    window.location.href="http://oa.gyyx.cn/signin/"
                }
            });
        }
    });
