...
$(function(){
    ...
    var serverlisturl = "/novice-oa/server/all";//服务器列表显示
                        ...
                        if (dCont && dCont.length && dCont[0]) {
                            for (var i = 0; i < dCont.length; i++) {
                                dataHtml += '<tr>'+
                                    ...
                                    '<td>'+dCont[i].name+'</td>'+
                                    '<td><a class="btn btn-info js_baseinfo" data_code="'+dCont[i].code+'"  data_name="'+dCont[i].name+'" href="#libaoTesk" data-toggle="modal" href="javascript:;">编辑</a><a class="btn btn-info js_manger" data_code="'+dCont[i].code+'"  data_name="'+dCont[i].name+'" href="#updateTask" data-toggle="modal" href="javascript:;">礼包管理</a><a class="btn btn-info js_create" data_code="'+dCont[i].code+'"  data_name="'+dCont[i].name+'" href="#newCardTask" data-toggle="modal">卡号生成</a></td>'+
                                    '</tr>';
            ...
            success:function(d){
                if(d.isSuccess){
                    ...
                    $("#table tbody").empty().html('<tr>'+
                        ...
                        '<td>'+d.data.name+'</td>'+
                        '<td><a class="btn btn-info js_baseinfo" data_code="'+d.data.code+'"  data_name="'+d.data.name+'" href="#libaoTesk" data-toggle="modal" href="javascript:;">编辑</a><a class="btn btn-info js_manger" data_code="'+d.data.code+'"  data_name="'+d.data.name+'" href="#updateTask" data-toggle="modal" href="javascript:;">礼包管理</a><a class="btn btn-info js_create" data_code="'+d.data.code+'"  data_name="'+d.data.name+'"  href="#newCardTask" data-toggle="modal">卡号生成</a></td>'+
                        '</tr>');
                }else{
                    alert(d.message);
                }
        ...
        $.ajax({
            ...
            success:function(d){
                if(d.isSuccess){
                    ...
                    if(serverisAll=="all"){
                        for(var i=0;i<d.data.length;i++){
                            serverName += '<option selected="selected" value="'+ d.data[i].code +'">'+ d.data[i].serverName +'</option>';
                        }
                    }else if(serverA.length==0){
                        for(var i=0;i<d.data.length;i++){
                        ...
                        //console.log( oldA)
                        for(var j=0;j<oldA.length;j++){
                            var elindex = oldA[j].substring(0,oldA[j].indexOf(":"));
                            var elqdname = oldA[j].substring(oldA[j].indexOf(":")+1);
                            serverName += '<option value="'+elindex+'">'+elqdname+'</option>';
                        }
    ...
    $("#js_footer").on("click",".js_save",function(){
        ...
        if($("#base_info .multiselect-container li.multiselect-all").hasClass("active")){//全选时状态
            serverName="all";
            serverId = -1;
        }
        if(pcNum == ""){
