...
//展示单个具体详情
    ...
    function getlist() {
        $("#js_tbSzList").ajaxPage({
            url: rc+"/challenteroa/getAllChallenterBean",//接口
            ...
            dataFn: function (data, $this) {
                ...
                if (dCont && dCont.length && dCont[0]) {
                    ...
                    for (var i = 0; i < dCont.length; i++) {
                        ...
                        dataHtml += '' +
                            '<tr>' +
                             ...
                             '  <td>' + dCont[i].oldRole  + '</td>' + //联系类型
                             '  <td>' + dCont[i].declaration + '</td>' +//联系方式
                    ...
                    $(".passUserInfo").live('click',function(){
                    	...
                    	$.ajax({
                    		url: rc+"/challenteroa/passChallengeInfo",
                            type: "POST",
                            data: { 
                            	r: Math.random(), 
                            	code:code
                    ...
                    $(".refuseUserInfo").live('click',function(){
                    	...
                    	$.ajax({
                    		url: rc+"/challenteroa/refuseChallengeInfo",
                            type: "POST",
                            data: { 
                            	r: Math.random(), 
                            	code:code
