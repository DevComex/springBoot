...
//展示单个具体详情
    ...
    function getlist() {
        $("#js_tbSzList").ajaxPage({
            url: rc+"/challenteroa/getAllUnPassChallenterUserInfo",//接口
            ...
            dataFn: function (data, $this) {
                ...
                if (dCont && dCont.length && dCont[0]) {
                    ...
                    for (var i = 0; i < dCont.length; i++) {
                        ...
                        dataHtml += '' +
                            '<tr>' +
                             ...
                             '  <td>' + dCont[i].contactType  + '</td>' + //联系类型
                             '  <td>' + dCont[i].contact + '</td>' +//联系方式
                    ...
                    $(".passUserInfo").live('click',function(){
                    	...
                    	$.ajax({
                    		url: rc+"/challenteroa/passUserInfo",
                            type: "POST",
                            data: { 
                            	r: Math.random(), 
                            	userCode:code
                    ...
                    $(".refuseUserInfo").live('click',function(){
                    	...
                    	$.ajax({
                    		url: rc+"/challenteroa/refuseUserInfo",
                            type: "POST",
                            data: { 
                            	r: Math.random(), 
                            	userCode:code
