jQuery.fn.extend({
	SeleHistory: function (options) {
        var Defaults = {
            AjaxSaleList: rc + "/stat/sale",
            DefaultMsgID: ".prompt",
            ErrorMsgID: ".error"
        };
        ...
        function pagelist(pageidx) {
        	...
        	if(m != "12"){
        		var endtime= myyear + '-' + (parseInt(m)+1) +"-1";
        	}else{
        		var endtime= (parseInt(myyear)+1) + "-1-1";
        	}
            $.ajax({
                type: "get",
                    ...
                    if (d.isSuccess) {
                        if (d.data.list == "") {
                            ...
                            for (var i = 0; i < d.data.list.length; i++) {
                                listtab += '<tr><td>' + d.data.list[i].userAccount + '</td><td>' + d.data.list[i].name + '</td><td>' + d.data.list[i].amount + '</td><td>' + d.data.list[i].gift + '</td><td>' + format(d.data.list[i].createTime, 'MM/dd') + '</td></tr>';
                            }
                            pIdx++;
                        ...
                        if (d.data.count.count == "0") {
                        	$(".js_sum").empty().html('总计：共  0  笔，销售量  0  张，赠送量  0 张');
                        }else{
                        	$(".js_sum").empty().html('总计：共 ' + d.data.count.count + ' 笔，销售量 ' + d.data.count.amountSum + ' 张，赠送量'+ d.data.count.giftSum+' 张');
                        }
                    } else {
                        alert(d.message);
                ...
                error: function () {
                   console.log("获取列表接口异常：/stat/sale");
                }
            });
        }
    },
