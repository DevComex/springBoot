jQuery.fn.extend({
	HeadManage: function (options) {
        ...
        $(".js_newhead").click(function(){
        	window.location.href = rc+"/head/new";
        });
        function pagelist(pageidx) {
            $.ajax({
                type: "get",
                url: Defaults.AjaxHeadList,
                data: {
                    ...
                    if (d.isSuccess) {
                        if (d.data == "") {
                            ...
                            for (var i = 0; i < d.data.length; i++) {
                            	...
                            	listtab += '<tr><td><a href="javascript:;" class="js_view" data-userid="' + d.data[i].account + '">' + d.data[i].account + '</a></td><td>' + names + '</td><td>' + (parseInt(d.data[i].cardInventory)+parseInt(d.data[i].cardGiftInventory)) + '</td><td><a href="javascript:;">充卡</a></td></tr>';
                            }
                            pIdx++;
