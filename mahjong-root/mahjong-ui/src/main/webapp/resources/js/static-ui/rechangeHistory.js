jQuery.fn.extend({
	RechargeHistory: function (options) {
        var Defaults = {
            AjaxRechList: rc + "/recharge/list",
            AjaxExchSury: rc + "/recharge/summary",
            DefaultMsgID: ".prompt",
            ErrorMsgID: ".error"
        };
        ...
        function pagelist(pageidx) {
            $.ajax({
                    ...
                    if (d.isSuccess) {
                        if (d.data == "") {
                            ...
                            for (var i = 0; i < d.data.length; i++) {
                                listtab += '<tr><td>' + d.data[i].operatorAccount + '</td><td>' + d.data[i].amount + '</td><td>' + format(d.data[i].createTime, 'MM/dd') + '</td></tr>';
                            }
                            pIdx++;
                ...
                error: function () {
                   console.log("列表接口异常：/recharge/list");
                }
            });
        }
        function getsummary() {
            $.ajax({
                ...
                success: function (d) {
                    if (d.isSuccess) {
                        if (d.data == "") {
                            $(".js_sum").empty().html('总计：购卡 0 笔，购卡数 0 张');
                        } else {
                            $(".js_sum").empty().html('总计：购卡 ' + d.data.count + ' 笔，购卡数 ' + d.data.sum + ' 张');
                        }
                    } else {
                        alert(d.message);
                        return false;
                ...
                error: function () {
                   console.log("统计接口异常：/recharge/summary");
                }
            });
        }
