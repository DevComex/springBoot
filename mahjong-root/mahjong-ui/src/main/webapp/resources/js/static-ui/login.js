jQuery.fn.extend({
    LoginValidate: function (options) {
            ...
            if (_VerificationNoPhone()) {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.isSuccess) {
                            window.location.href = rc+ "/exchange/"
                        } else {
                            alert(data.message);
        ...
        $(".js_kbq").click(function () {
            $(".js_capimg").attr("src", rc + "/captcha/create?bid=lg&r=" + Math.random());
        });
        $(".js_capimg").click(function () {
            $(this).attr("src", rc + "/captcha/create?bid=lg&r=" + Math.random());
        });
        ...
        function isNeedCaptcha(acc) {
            $.ajax({
                ...
                error: function () {
                    console.log("是否需要验证码接口异常：/login/isNeedCaptcha");
                }
            });
        }
        var _VerCheckPassword = function (obj) {
            if ($.trim(obj.val()).length > 16 || $.trim(obj.val()).length < 6) {
                ...
                obj.parents(".item_p").find(".error").css("display", "inline-block").html("<b>密码过短，必须在6-16个非空格字符之间</b>");
                return false;
            } else {
                return true;
            }
