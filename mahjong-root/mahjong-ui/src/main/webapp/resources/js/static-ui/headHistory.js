                    ...
                    if (d.isSuccess) {
                        if (d.data == "") {
                            ...
                            for (var i = 0; i < d.data.length; i++) {
                                listtab += '<tr><td>' + d.data[i].gameUserId + '</td><td>' + d.data[i].gameNickName + '</td><td>' + d.data[i].amount + '</td><td>' + format(d.data[i].createTime, 'MM/dd') + '</td><td><a href="javascript:;">充卡</a></td></tr>';
                            }
                            pIdx++;
        ...
        function getsummary() {
            $.ajax({
                ...
                success: function (d) {
                    if (d.isSuccess) {
                        if (d.data == "") {
                            $(".js_sum").empty().html('总计：售卡 0 笔，售卡数 0 张');
                        } else {
                            $(".js_sum").empty().html('总计：售卡 ' + d.data.count + ' 笔，售卡数 ' + d.data.sum + ' 张');
                        }
                    } else {
                        alert(d.message);
                        return false;
                ...
                error: function () {
                    console.log("获取统计信息接口异常：/exchange/summary");
                }
            });
        }
