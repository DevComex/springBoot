$(function () {
    $.getScript("../resources/js/menu/data/headtxt.txt?r=" + Math.random(), function () {
        var obj = bottomdata;
        var bottommenulist = "", bmlyer = "";
        for (var i = 0; i < obj.length; i++) {
            if (i == 0) {
                bottommenulist += '<a href="javascript:;" class="l">' + obj[i].FirName + '</a>';
                bmlyer += '<div class="overlay l l' + obj[i].Code + ' overlay' + obj[i].Code + '"><nav><ul>';
            } else {
                bottommenulist += '<a href="javascript:;" class="leftline l">' + obj[i].FirName + '</a>';
                bmlyer += '<div class="overlay l l' + obj[i].Code + ' overlay' + obj[i].Code + '"><nav><ul>';
            }
        ...
        function loginSession() {
            $.ajax({
                ...
                error: function () {
                    console.log("判断登陆状态接口异常：/login/session");
                }
            });
        }
        if (window.location.pathname.indexOf("findpwd")<0) {
        ...
        function logout() {
            $.ajax({
                ...
                error: function () {
                    console.log("注销接口异常：/user/logout");
                }
            });
        }
