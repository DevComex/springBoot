<%@ WebHandler Language="C#" Class="ImageUpload.imageUp" %>
using System;
using System.Web;
using System.IO;
    ...
    public class imageUp : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            if (!String.IsNullOrEmpty(context.Request.QueryString["fetch"]))
            {
                ...
                context.Response.Write(String.Format("updateSavePath([{0}]);", String.Join(", ", ImageUpload.ConfigBaiDu.ImageSavePath.Select(x => "\"" + x + "\""))));
                return;
            }
            ...
            ImageUpload.Uploader up = new ImageUpload.Uploader();

            string path = up.getOtherInfo(context, "dir");
            if (String.IsNullOrEmpty(path))
            {
                path = ImageUpload.ConfigBaiDu.ImageSavePath[0];
            }
            else if (ImageUpload.ConfigBaiDu.ImageSavePath.Count(x => x == path) == 0)
            {
