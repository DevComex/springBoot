...
$.fn.extend({
    ...
    getChapterList:function (options){
        ...
        pagingFn({
            ...
            dataFn:function(d){
                if(d.isSuccess){
                    var listhtml = ""; var account = getParam().name;
                    var listhtml = '' +
                                    '<tr>' +
                                    '    <th>' +
                   ...
                   if(d.dataSet.length>0){
                	   for (var i = 0; i < d.dataSet.length; i++) {
                           ...
                           var isdisplay = d.dataSet[i].englishStatus;
                           var button ='';
                           var chapterCode=d.dataSet[i].code;
                        	   if(d.dataSet[i].englishStatus == "passed"){
                        		   if(account=="官方"){
                        			   button =
                                           '<a href="javascript:;" class="btn btn-xs btn-primary  js_check" title="查看" data-name = "'+account+'">查看' +
                        			   ...
                        			   button =
                                           ...
                                           '</a>'+
                                           '<a data-id="editAc" href="javascript:"  class="btn btn-xs btn-info " title="发奖">发奖'+
                                               
                                           '</a>'
                        		   }
                        		   
                            	  
                               }else if(d.dataSet[i].englishStatus == "unreviewd"){
                            	   //审核的按钮
                            	   if(!d.dataSet[i].prevStatus){
                            		   button =
                                    	   '<a href="javascript:;" class="btn btn-xs btn-primary  js_review grey" title="审核" style="cursor:default;">审核' +
                                           
                                           '</a>'
                            	   }else{
                            		   button= '<a href="javascript:;" class="btn btn-xs btn-primary  js_review" title="审核" data-name="'+account+'">审核' +
                                       
                                       '</a>'
                            	   }
                            	   button +='<a data-id="editAcs" href="/novel/chapter/editor?chapterCode='+chapterCode+'" target="_blank"  class="btn btn-xs btn-info js_edit" title="编辑">编辑'+
                                       
                                   '</a>'
                               }else if(d.dataSet[i].englishStatus == "failed"){
                            	   button =
                            	   '<a href="javascript:;" class="btn btn-xs btn-primary  js_fail_check" title="查看" data-name = "'+account+'">查看' +
                           ...
                           '    <td>'+d.dataSet[i].chapterNum+'</td>'+
                           '    <td>'+d.dataSet[i].serverName+'</td>'+
                           '    <td>' + d.dataSet[i].roleName + '</td>' +
                           '    <td>' + d.dataSet[i].title + '</td>' +
                           '    <td>' + d.dataSet[i].createTime + '</td>' +
                           '    <td>' + d.dataSet[i].chineseStatus+ '</td>' +
                           '    <td>'+
                                   button+
                           '    </td>'
                           '</tr>';
            ...
            successFn: function (allCount, eachCount, pgindex) {
                ...
                //点击审核通过的查看按钮
          	  $("#js_ArticleList .js_check,#js_ArticleList .js_fail_check").click(function(){
                	var name = $(this).data("name");
                	var viewCode=$(this).parents('tr').attr('data-id');
                	location.href ="/novel/chapter/view/"+viewCode+"?name="+name;
                	
                })
               
               //点击待审核的按钮
               $("#js_ArticleList .js_review").click(function(){
            	   var name = $(this).data("name");
            	   if($(this).hasClass("grey")){
            	    ...
            	    var viewCode=$(this).parents('tr').attr('data-id');
            	    location.href ="/novel/chapter/view/"+viewCode+"?name="+name;
                	//location.href ="/novel/chapter/view/"+viewCode;
