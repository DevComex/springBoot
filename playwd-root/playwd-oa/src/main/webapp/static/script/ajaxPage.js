...
$.fn.extend({
    ...
    ajaxPage: function (options) {
        ...
        var ajaxSuc = function (data) {
            if (data) {
                ...
                var
                //总数量
                    allCount = setting.pageData ? eval(setting.pageData) : (data.dataSet && data.count),
                //总页数
                    allPage = 0;
                //如果能取到总数量并且是大于0整数
                ...
                if (setting.successFn) {
                    ...
                    $(".play0").on("click",function(){
                        var attFlash=new RegExp(".flv");
                        var contA=$(this).attr("videourl");
                        if(!attFlash.test(contA))
                        {
                        ...
                        if(attFlash.test(contA))
                        {
                            ...
                            YM.Flash.embedAdPlayer({
                                ...
                                height: 360,
                                vars: {
                                    //http://download.gyyx.cn/Default.ashx?typeid=941%5EnetType=1%5Efile=summer.flv
                                    videoUrl:contA,
                                    coverUrl: ''
                                }
                            });
    ...
    pagesFn: function (setting) {
        ...
        //分页按钮html容器
            pagehtml = '<a ' + (isfirstpg ? 'class="first paginate_button paginate_button_disabled"' : 'class="first paginate_button"') + ' href="javascript:;"  pgid="-4">' + btnTxt.prev + '</a>' +
                '<a ' + (isfirstpg ? 'class="paginate_active"' : 'class="paginate_button"') + ' href="javascript:;" pgid="-3">' + btnTxt.first + '</a>';
        ...
        if (allPage < 6) {//如果页面小于6页时，显示全部页码
            ...
            if (pageIndex < 4) {//如果当前页小于4时，可见数量最大为5然后加...
                ...
                for (var i = 0; i < 2 ; i++) {
                    pagehtml += '<a  class="paginate_button" href="javascript:;" pgid="' + (i + 1) + '">' + (i + 1) + '</a>';
                }
                pagehtml += aDot;
        ...
        pagehtml += '<a ' + (lastpg ? 'class="paginate_active"' : 'class="paginate_button"') + ' href="javascript:;" pgid="-2">' + btnTxt.last + '</a>' +
