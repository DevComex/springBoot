    ...
    ajaxPage: function (options) {
        ...
        var ajaxSuc = function (data) {
            if (data) {
                ...
                var
                    //总数量
                    allCount = setting.pageData ? eval(setting.pageData) : (data.data.list && data.data.total)
                   // allCount = 7,
                    //总页数
                    //allPage = 0;
                //如果能取到总数量并且是大于0整数
    ...
    pagesFn: function (setting) {
        var
            ...
            pagehtml = '<a ' + (isfirstpg ? 'class="first paginate_button paginate_button_disabled"' : 'class="first paginate_button"') + ' href="javascript:;"  pgid="-4">' + btnTxt.prev + '</a>' +
                                  '<a ' + (isfirstpg ? 'class="homePage"' : 'class="paginate_button"') + ' href="javascript:;" pgid="-3">' + btnTxt.first + '</a>';
