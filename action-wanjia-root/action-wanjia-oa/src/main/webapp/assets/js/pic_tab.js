...
jq.fn.banqh = function(can){
	...
	function showPic(){
        ...
        $.ajax({
           url: rc+"/review/showBookInfoAndPage?bookCode="+book_code, 
            type: "GET",
            dataType: "json",
            data: {
                r: Math.random
            ...
            success: function (data) {
            	var Data =data.data
                var previewPopLi = "";
                var previewPop_smallLi = "";
                if(data.success){
                	if (Data.pageList.length && Data) {
                        for (var i = 0; i < Data.pageList.length; i++) {
                            previewPopLi += '<li><a href="javascript:;"><img src="' + Data.pageList[i].page_picture_url.replace("-small","") + '" width="500" height="500"  /></a></li>'
                            previewPop_smallLi += '<li><a href="javascript:;"><img src="' + Data.pageList[i].page_picture_url + '" width="80" height="79"/></a></li>'

                        };
                        $("#previewPop").empty().append(previewPopLi);
                        $("#previewPop_small").empty().append(previewPop_smallLi);
                    };
            		...
            		jq(can.pnum).find('ul').width(picminnum*picminw + 35).height(picminnum*picminh);
	...
	//预览图片
	$(".js_watchPic").click(function(){
		$(".Mask").show();
		$(".js_popKuang").show();
		var previewPopLi='',previewPop_smallLi='';
            $(".manhuaPic img").each(function(){
            	previewPopLi += '<li><a href="javascript:;"><img src="' + $(this).attr("src") + '" width="500" height="500"  /></a></li>';
            	previewPop_smallLi += '<li><a href="javascript:;"><img src="' + $(this).attr("src") + '" width="80" height="79"/></a></li>';
            })
            $("#previewPop").empty().append(previewPopLi);
            $("#previewPop_small").empty().append(previewPop_smallLi);
